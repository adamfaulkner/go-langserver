...
func getAsYouTypeFiles(origFilename string, modifiedFilename string) ([]*ast.File, error) {
...
func checkPkgFiles(files []*ast.File) []error {
	type bailout struct{}
...
func CheckFile(origFilename, modifiedFilename string) ([]error, error) {
	files, err := getAsYouTypeFiles(origFilename, modifiedFilename)
	if err != nil {
		return nil, err
	}
	return checkPkgFiles(files), nil
}
...
func main() {
	...
	if *modifiedFilename != "" || *origFilename != "" {
		if *modifiedFilename == "" || *origFilename == "" {
			report([]error{errors.New("Both or neither modifiedFilename and origFilename must be specified.")})
			os.Exit(2)
		}
